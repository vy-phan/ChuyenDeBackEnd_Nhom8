...
- **Bảo vệ định tuyển và tạo token jwt**.
  - [`protectToken`](backend/middleware/protectToken.js)
  - [`generateToken`](backend/utils/generateToken.js)

- **Dịch vụ Email**: Gửi email xác minh sử dụng NodeMailer.
...
1. **Backend**:
   ...
   ```
        ...
        EMAIL_USER=email cần để gửi email
        EMAIL_APP_PASSWORD=mã mật khẩu được google cấp quyền
   ```
